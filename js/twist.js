...
import { find_cuts, find_stops, make_move } from './move.js';
import { triangulate_polygeometry } from './slice.js';
...
var arrow_geometry = function () {
    ...
    function lineto_polar(shape, r, theta) { shape.lineTo(r * Math.cos(theta), r * Math.sin(theta)); }
...
function draw_puzzle(newpuzzle, scene) {
...
function draw_arrow(cut) {
    let arrow_material = new THREE.MeshPhongMaterial({transparent: true, opacity: 0.3, side: THREE.DoubleSide});
...
function highlight_arrow(puzzle, mouse) {
    if (mouse === null) return;
    if (mouseover_arrow !== null) {
        mouseover_arrow.material.color.set(0xFFFFFF);
        mouseover_arrow.material.opacity = 0.3;
    }
    raycaster.setFromCamera(mouse, camera);
    let intersects = raycaster.intersectObjects(arrows);
    if (intersects.length > 0) {
        mouseover_arrow = intersects[0].object;
        mouseover_arrow.material.color.set(0xFFFFCC);
        mouseover_arrow.material.opacity = 0.9;
    }
}

// bug: when a drag ends inside an arrow, it counts as a click
function onmousemove(event) {
    if (event.target !== renderer.domElement) return;
    if (mouse === null) mouse = new THREE.Vector2();
...
function onclick(event) {
    ...
    let intersects = raycaster.intersectObjects(arrows);
    if (intersects.length > 0) {
        let i = arrows.findIndex(c => c === intersects[0].object);
...
document.getElementById('new_cut').addEventListener('click', function (e) {
    ...
    let cut_item = cut_menu.firstChild.cloneNode(true);
...
document.getElementById('apply_cuts').addEventListener('click', function (e) {
    let shell_menu = document.getElementById("shell_shape");
    let shell_shape = shell_menu.options[shell_menu.selectedIndex].value;

    let shell, inradius, circumradius;
    switch(shell_shape) {
    ...
    let cut_menu = document.getElementById("cuts");
    for (let li of cut_menu.childNodes) {
        // skip first item, which is a dummy item
        if (li === cut_menu.firstChild) continue;
        let shape, distance;
        for (let e of li.childNodes) {
            if (e.className == "cut_shape")
                shape = e.options[e.selectedIndex].value;
            else if (e.className == "cut_distance")
                distance = e.value;
        }
        switch(shape) {
...
document.getElementById('scramble').addEventListener('click', function (e) { scramble(20); });
...
var cur_move = null;
function begin_move(cut, angle, callback) {
...
function end_move() {
    ...
    for (let i=0; i<cur_move.pieces.length; i++)
        puzzle[cur_move.pieces[i]].object.quaternion.copy(puzzle[cur_move.pieces[i]].rot);
    cur_move = null;
    draw_arrows(puzzle);
...
function animate(t) {
    if (cur_move !== null) {
        if (cur_move.start_time === undefined)
